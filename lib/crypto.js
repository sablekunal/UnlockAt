...
export function encryptBuffer(buffer, key) {
    ...
    const encryptedData = Buffer.concat([
        ...
        cipher.final(),
        cipher.getAuthTag() // Append 16-byte tag to the data
    ]);
 ...
 * @param {Buffer} encryptedDataWithTag 
 * @param {Buffer} key 
 * @param {Buffer} iv 
 * @returns {Buffer} Original data.
 */
export function decryptBuffer(encryptedDataWithTag, key, iv) {
    // Slice off the last 16 bytes (the GCM auth tag)
    const tag = encryptedDataWithTag.subarray(-16);
    const data = encryptedDataWithTag.subarray(0, -16);
